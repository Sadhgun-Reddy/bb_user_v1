...
const MOCK_DATA = {
    kpiStats: [
        { id: 1, title: 'Profile Views', value: '1,240', subtext: 'this month', badge: '+12%', badgeIcon: TrendingUp, badgeColor: 'text-green-700 bg-green-100', isPrimary: false },
        { id: 2, title: 'New Enquiries', value: '8', subtext: 'unread', badge: 'New', badgeColor: 'text-[#ef9d2a] bg-[#ef9d2a]/20', isPrimary: true },
        { id: 3, title: 'Open Enquiries', value: '3', subtext: 'in progress', badge: null, isPrimary: false },
        { id: 4, title: 'Response Rate', value: '94%', subtext: 'excellent', badge: 'Avg 2h', badgeColor: 'text-gray-600 bg-gray-100', isPrimary: false },
    ],
    recentEnquiries: [
        {
            id: 1,
        ...
        <div className="flex flex-col gap-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-stone-500 text-lg">Here's what's happening with your catering business today.</p>
                </div>
            ...
            <div className="w-full bg-orange-50 border border-orange-200 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 shadow-sm">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-stone-600 mt-1">Your updated documents are currently under review. Your profile visibility is limited until approval (approx. 24-48h).</p>
                </div>
                <a href="#" className="text-sm font-bold text-orange-700 hover:text-orange-800 whitespace-nowrap flex items-center gap-1 group transition-colors">
                    Check Status
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {MOCK_DATA.kpiStats.map((stat) => (
                    <div key={stat.id} className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden min-h-[140px]">
                        ...
                        <div className="flex items-center justify-between relative z-10 mb-4">
                            ...
                            {stat.badge && (
                                <div className={`${stat.badgeColor} text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1`}>
                        ...
                        <div className="flex items-end gap-2 relative z-10">
                            <h2 className={`text-3xl font-bold ${stat.isPrimary ? 'text-[#ef9d2a]' : 'text-[#1b160d]'}`}>
                                {stat.value}
                            </h2>
                ...
                <div className="lg:col-span-2 flex flex-col gap-4">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden flex flex-col">
                        {MOCK_DATA.recentEnquiries.map((enquiry) => (
                            <div key={enquiry.id} className="p-5 border-b border-stone-50 hover:bg-stone-50 transition-colors flex flex-col sm:flex-row gap-4 sm:items-center justify-between group last:border-0">

                                <div className="flex items-start gap-4">
                                    <div className={`w-12 h-12 rounded-full ${enquiry.iconColor} flex items-center justify-center shrink-0`}>
                                        <enquiry.icon className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="font-bold text-base text-[#1b160d]">{enquiry.type}</h3>
                                            <span className={`${enquiry.statusColor} text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider`}>
                                                {enquiry.status}
                                            </span>
                                        </div>
                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-stone-500 font-medium">
                                            <span className="flex items-center gap-1"><CalendarClock className="w-4 h-4" /> {enquiry.date}</span>
                                            <span className="flex items-center gap-1"><Users className="w-4 h-4" /> {enquiry.guests} Guests</span>
                                            <span className="flex items-center gap-1"><User className="w-4 h-4" /> {enquiry.customer}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 h-full flex flex-col">
                        <div className="grid grid-cols-1 gap-3 flex-1">
                            ...
                            <button className="flex items-center gap-4 p-4 rounded-xl bg-stone-50 hover:bg-orange-50 hover:border-orange-200 border border-transparent transition-all group text-left">
                                ...
                                <div>
                                    <span className="block font-bold text-[#1b160d] group-hover:text-[#ef9d2a] transition-colors">Upload new dish</span>
                            ...
                            <button className="flex items-center gap-4 p-4 rounded-xl bg-stone-50 hover:bg-orange-50 hover:border-orange-200 border border-transparent transition-all group text-left">
                                ...
                                <div>
                                    <span className="block font-bold text-[#1b160d] group-hover:text-[#ef9d2a] transition-colors">Update availability</span>
                            ...
                            <button className="flex items-center gap-4 p-4 rounded-xl bg-stone-50 hover:bg-orange-50 hover:border-orange-200 border border-transparent transition-all group text-left">
                                ...
                                <div>
                                    <span className="block font-bold text-[#1b160d] group-hover:text-[#ef9d2a] transition-colors">Share profile</span>
                        ...
                        <div className="mt-6 pt-4 border-t border-stone-100">
                            <p className="text-xs text-stone-400 mb-3 uppercase tracking-wider font-bold">Support</p>
                            <a href="#" className="flex items-center gap-2 text-sm font-medium text-stone-600 hover:text-[#ef9d2a] transition-colors">
