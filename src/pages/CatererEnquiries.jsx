...
const getAcquisitionBadge = (type) => {
    return type === 'Direct'
        ? <span className="px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider bg-purple-100 text-purple-700">Direct</span>
        : <span className="px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-700">Broadcast</span>;
};
    ...
    <div className="flex flex-col gap-6 mb-8">
        ...
        <div className="flex flex-col gap-1">
            <div className="flex items-center gap-4">
                <h1 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">Leads & Enquiries</h1>
            </div>
            <p className="text-stone-500 font-medium">
                You have <strong className="text-[#ef9d2a]">{newLeadsCount} new leads</strong> awaiting your response today.
            </p>
        </div>
    ...
    <div className="relative group flex flex-col lg:flex-row lg:items-center justify-between p-5 lg:p-6 border-b border-stone-100 hover:bg-orange-50/30 transition-colors cursor-pointer bg-white first:rounded-t-[2.5rem] last:rounded-b-[2.5rem] last:border-b-0 gap-4 lg:gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6 flex-1 min-w-0 lg:pr-6 pl-4 lg:pl-0">
            ...
            <div className="flex items-center gap-4 w-full sm:w-64 shrink-0">
                <div className="relative shrink-0">
                    {lead.customer.avatar ? (
                        <img src={lead.customer.avatar} alt={lead.customer.name} className="w-12 h-12 rounded-full object-cover border border-stone-200 shadow-sm" />
                    ) : (
                ...
                <div className="flex flex-col min-w-0">
                    <span className={`text-[15px] truncate ${lead.status === 'New' ? 'font-black text-stone-900' : 'font-bold text-stone-700'}`}>
                        {lead.customer.name}
                    </span>
            ...
            <div className="flex flex-col gap-1.5 w-full sm:w-64 shrink-0 sm:border-l sm:border-stone-100 sm:pl-6">
                <span className={`text-[15px] truncate ${lead.status === 'New' ? 'font-bold text-stone-900' : 'font-semibold text-stone-700'}`}>
                    {lead.logistics.event}
                </span>
                <div className="flex items-center gap-2 text-xs font-bold text-stone-500">
                    <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" />{lead.logistics.date}</span>
                    <span className="w-1 h-1 rounded-full bg-stone-300"></span>
                    <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" />{lead.logistics.guests}</span>
                </div>
            </div>
                    ...
                    {filteredLeads.length > 0 ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center my-auto">
                            ...
                            <p className="text-stone-500 font-medium max-w-sm">
                                {searchQuery ? 'Try adjusting your search filters.' : "You have no active leads in your inbox right now."}
                            </p>
                        </div>
                    )}
                </div>
