...
type SupportFormInputs = Pick<SupportTicket, 'subject' | 'message'> & {
  name: string;
  email: string;
};
...
const FAQ_LIST = [
  {
    ...
    answer:
      'You can cancel a booking directly from your Enquiries Dashboard if the status is still "In Progress". If the booking is "Confirmed", you will need to contact the caterer directly as per their specific cancellation policy.',
  },
  {
    id: 'faq-2',
    question: 'When do caterers get paid?',
    answer:
      'Book Bawarchi holds the initial deposit in escrow. The caterer receives the deposit 48 hours before the event date, and the remaining balance is settled directly between you and the caterer post-event.',
  },
  {
    id: 'faq-3',
    question: 'How do I know if a caterer is verified?',
    answer:
      'All verified caterers undergo a strict background check, including FSSAI license verification. Look for the blue "Verified" shield badge on their profile.',
  },
  {
    id: 'faq-4',
    question: "What happens if a caterer doesn't show up?",
    answer:
      'We have a strict Service Level Agreement. In the rare event of a no-show, you will receive a 100% refund of your deposit, and our emergency support team will attempt to find a replacement vendor immediately.',
  },
];
...
const CONTACT_METHODS = [
  {
    id: 'cm-1',
    type: 'Email Us',
    detail: 'support@bookbawarchi.com',
  ...
  },
  {
    id: 'cm-3',
    type: 'Head Office',
    detail: '124 Innovation Drive',
    subtext: 'Koramangala, Bangalore 560034',
    icon: MapPin,
  },
];
...
const HeroHeader: React.FC = () => (
  <div className="flex flex-col gap-4 mb-12">
    ...
    <p className="text-lg md:text-xl text-slate-600 font-medium max-w-2xl leading-relaxed">
      Need assistance with a booking, have a question about a caterer, or experiencing technical
      issues? Our support team is ready.
    </p>
  </div>
);

const ContactForm: React.FC = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<SupportFormInputs>();

  const onSubmit: SubmitHandler<SupportFormInputs> = (data) => {
    console.log("Support Ticket Submitted", data);
    // Future: Dispatch action to create ticket
  };
      ...
      <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-semibold text-stone-900 focus:ring-2 focus:ring-[#ef9d2a]/20 outline-none transition-shadow appearance-none cursor-pointer"
            defaultValue=""
          >
          ...
          {errors.subject && <span className="text-red-500 text-xs ml-2">{errors.subject.message}</span>}
        </div>
        ...
        <button type="submit" className="w-full md:w-auto mt-2 px-10 py-4 text-lg rounded-full bg-[#ef9d2a] hover:bg-[#d98a1e] text-white font-black flex items-center justify-center gap-3 shadow-md shadow-orange-500/20 active:scale-95 transition-all self-start">
          Send Message
...
const TicketRedirectCard: React.FC = () => (
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm font-medium text-slate-600">
          View the status of your existing open tickets.
        </p>
      </div>
    </div>
    <Link
      to="/dashboard"
      className="px-8 py-3 rounded-full bg-white border-2 border-orange-200 text-[#ef9d2a] hover:bg-orange-50 font-black text-sm transition-colors text-center whitespace-nowrap"
    >
      Go to Dashboard
    </Link>
  </div>
    ...
    <div className="flex flex-col gap-6">
      {CONTACT_METHODS.map((method) => {
        ...
        return (
          <div
            key={method.id}
            className="flex gap-5 p-4 rounded-2xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-colors group"
          >
            ...
            <div className="flex flex-col justify-center">
              <span className="text-xs font-black uppercase tracking-widest text-[#ef9d2a] mb-0.5">
                {method.type}
              </span>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <h3
                  className={`font-bold transition-colors select-none text-base sm:text-[17px] ${isOpen ? 'text-[#ef9d2a]' : 'text-stone-900'}`}
                >
                  {faq.question}
                </h3>
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors ${isOpen ? 'bg-orange-100 text-[#ef9d2a]' : 'text-slate-400'}`}
                >
