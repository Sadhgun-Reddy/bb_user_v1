...
const MOCK_DATA = {
    stats: [
        { id: 1, title: 'Active Enquiries', value: '3', badge: '+2 new', icon: FolderOpen, badgeColor: 'text-green-600 bg-green-50' },
        { id: 2, title: 'New Responses', value: '12', badge: 'Action Req.', icon: MessageCircle, badgeColor: 'text-[#ef9d2a] bg-orange-50' },
    ...
    recentEnquiries: [
        { id: 1, title: 'Birthday Bash', date: 'Dec 24, 2023', guests: 150, status: 'Pending', icon: Cake, statusColor: 'bg-yellow-100 text-yellow-700' },
        { id: 2, title: 'Corporate Lunch', date: 'Jan 10, 2024', guests: 50, status: 'Confirmed', icon: Users, statusColor: 'bg-green-100 text-green-700' },
        { id: 3, title: 'Wedding Reception', date: 'Feb 14, 2024', guests: 300, status: 'Draft', icon: Utensils, statusColor: 'bg-stone-200 text-stone-600' },
    ],
    adminNotes: [
        { id: '4092', text: '"Customer requested gluten-free options for the dessert menu. Please update the proposal."', time: '2h ago', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&q=80&w=150' },
        { id: '3021', text: '"Menu tasting scheduled for Friday at 2 PM. Venue address updated."', time: '1d ago', avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=150' },
    ],
    savedCaterers: [
        { id: 1, name: 'Spice Symphony', rating: '4.9', reviews: 120, tags: 'Indian, Mughlai', image: 'https://images.unsplash.com/photo-1555244162-803834f70033?auto=format&fit=crop&q=80&w=150' },
        { id: 2, name: 'Urban Platter Co.', rating: '4.7', reviews: 85, tags: 'Continental', image: 'https://images.unsplash.com/photo-1577219491135-ce391730fb2c?auto=format&fit=crop&q=80&w=150' },
    ],
    supportTickets: [
        { id: 'TK992', title: 'Payment Query', desc: 'Waiting for support team response.', status: 'In Progress' }
    ]
};
        ...
        <div className="flex flex-col gap-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {MOCK_DATA.stats.map((stat) => (
                    <div key={stat.id} className="group relative flex flex-col justify-between overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-100 transition-all hover:-translate-y-1 hover:shadow-lg">
                        <div className="flex items-start justify-between">
                            <div className="rounded-full bg-orange-50 p-3 text-[#ef9d2a] transition-transform group-hover:scale-110">
                                <stat.icon className="w-6 h-6" />
                            </div>
                            {stat.badge && (
                                <span className={`flex items-center text-xs font-bold px-2 py-1 rounded-full ${stat.badgeColor}`}>
                                    {stat.badge}
                                </span>
                            )}
                        </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div className="flex flex-col rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-100">
                    <div className="mb-6 flex items-center justify-between">
                        ...
                        <a href="#" className="text-sm font-bold text-[#ef9d2a] hover:text-orange-600 transition-colors">View All</a>
                    </div>
                    <div className="flex flex-col gap-4">
                        {MOCK_DATA.recentEnquiries.map((enquiry) => (
                            <div key={enquiry.id} className="flex items-center justify-between rounded-xl border border-stone-100 bg-stone-50/50 p-4 transition hover:bg-stone-50">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-stone-500">{enquiry.date} &bull; {enquiry.guests} Guests</p>
                                    </div>
                                </div>
                ...
                <div className="flex flex-col rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-100">
                    ...
                    <div className="flex flex-col gap-4">
                        {MOCK_DATA.adminNotes.map((note) => (
                            <div key={note.id} className="rounded-xl border border-dashed border-stone-200 p-4 bg-stone-50/30">
                                <div className="mb-2 flex items-center gap-2">
                                    <StickyNote className="w-4 h-4 text-[#ef9d2a]" />
                                    <span className="text-xs font-bold uppercase tracking-wider text-stone-400">Request #{note.id}</span>
                                </div>
                                ...
                                <div className="mt-3 flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-stone-500 font-medium">Admin Note &bull; {note.time}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex flex-col rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-100">
                    <div className="mb-6 flex items-center justify-between">
                        ...
                        <a href="#" className="text-sm font-bold text-[#ef9d2a] hover:text-orange-600 transition-colors">Browse All</a>
                    </div>
                    <div className="flex flex-col gap-4">
                        {MOCK_DATA.savedCaterers.map((caterer) => (
                            <div key={caterer.id} className="flex items-center gap-4 rounded-xl p-2 transition hover:bg-stone-50 cursor-pointer">
                ...
                <div className="flex flex-col rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-100">
                    ...
                    <div className="space-y-4 flex-1 flex flex-col justify-start">
                        {MOCK_DATA.supportTickets.map((ticket, idx) => (
                            <div key={idx} className="flex items-start gap-3 border-l-4 border-yellow-400 bg-stone-50 p-4 rounded-r-xl shadow-sm">
                                <div className="flex-1">
                                    <h4 className="text-sm font-bold text-[#1b160d]">{ticket.title} - #{ticket.id}</h4>
