...
const DonationLifecycle = () => {
    const steps = [
        { id: 1, title: 'Share Details', icon: <HeartHandshake className="w-6 h-6 text-[#ef9d2a]" />, bg: 'bg-orange-50', border: 'border-orange-100' },
        { id: 2, title: 'We Verify', icon: <ShieldCheck className="w-6 h-6 text-blue-500" />, bg: 'bg-blue-50', border: 'border-blue-100' },
        { id: 3, title: 'We Collect', icon: <Truck className="w-6 h-6 text-slate-700" />, bg: 'bg-slate-100', border: 'border-slate-200' },
    ];
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 mb-12 relative w-full px-4">
            ...
            {steps.map((step, index) => (
                <div key={step.id} className="flex flex-col items-center gap-3 relative bg-white sm:px-4">
                    <div className={`w-12 h-12 rounded-2xl ${step.bg} ${step.border} border flex items-center justify-center shadow-sm`}>
                        {step.icon}
                    </div>
...
const PhotoUploadWidget = ({ file, onDrop, onRemove }) => {
    return (
        <div className="relative group">
            ...
            <div className={`w-full border-2 ${file ? 'border-[#ef9d2a]' : 'border-dashed border-slate-300 group-hover:border-[#ef9d2a]/50'} bg-slate-50 rounded-2xl p-8 transition-colors flex flex-col items-center justify-center text-center gap-4`}>
                {!file ? (
                    <>
...
const InputField = ({ label, icon: Icon, type = "text", placeholder, options, ...props }) => {
    const inputClasses = "w-full bg-slate-50 border-transparent focus:border-[#ef9d2a]/30 focus:bg-white focus:ring-4 focus:ring-[#ef9d2a]/10 rounded-2xl px-5 py-4 text-sm font-medium text-slate-900 placeholder:text-slate-400 transition-all shadow-sm outline-none";
    ...
    return (
        <div className="flex flex-col gap-2 relative">
            ...
            <div className="relative">
                ...
                {type === 'select' ? (
                    <select
                        className={`${inputClasses} appearance-none ${Icon ? 'pl-12' : ''}`}
                        {...props}
                    >
                        <option value="" disabled className="text-slate-400">{placeholder}</option>
                        {options?.map(opt => (
                            <option key={opt.value} value={opt.value} className="text-slate-900 py-2">{opt.label}</option>
                        ))}
                    </select>
                ) : type === 'textarea' ? (
                    <textarea
        ...
        <div className="min-h-screen bg-[#fcfaf8] font-['Plus_Jakarta_Sans',sans-serif] selection:bg-[#ef9d2a]/30 py-12 px-4 sm:px-6 lg:px-8 flex justify-center">
            ...
            <div className="w-full max-w-4xl bg-white rounded-[3rem] shadow-2xl shadow-stone-200/50 outline outline-1 outline-stone-100 overflow-hidden">
                ...
                <div className="px-8 sm:px-16 pt-16 pb-8 text-center bg-gradient-to-b from-orange-50/50 to-white">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight mb-4">Donate Food</h1>
                    <p className="text-lg text-slate-500 font-medium max-w-xl mx-auto">
                        List your surplus food. We'll verify it and match it with local community centers or those in urgent need.
                    </p>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-12 mt-8">
                        ...
                        <div className="flex flex-col gap-6">
                            <h3 className="text-lg font-black text-[#ef9d2a] flex items-center gap-3">
                                <span className="flex items-center justify-center w-7 h-7 rounded-full bg-[#ef9d2a] text-white text-sm">1</span>
                                Contact Information
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <InputField label="Full Name" placeholder="e.g. Jane Doe" value={formData.fullName} onChange={e => setFormData({ ...formData, fullName: e.target.value })} />
                                <InputField label="Phone Number" type="tel" placeholder="+1 (555) 000-0000" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                                <div className="md:col-span-2">
                                    <InputField label="Email Address" type="email" placeholder="jane@example.com" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-6">
                            <h3 className="text-lg font-black text-[#ef9d2a] flex items-center gap-3">
                                <span className="flex items-center justify-center w-7 h-7 rounded-full bg-[#ef9d2a] text-white text-sm">2</span>
                                Donation Details
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <InputField
                            ...
                            <div className="mt-2 text-left">
                                <label className="text-[13px] font-bold text-slate-700 ml-2 mb-2 block">Proof of Donation / Image</label>
                        ...
                        <div className="flex flex-col gap-6">
                            <h3 className="text-lg font-black text-[#ef9d2a] flex items-center gap-3">
                                <span className="flex items-center justify-center w-7 h-7 rounded-full bg-[#ef9d2a] text-white text-sm">3</span>
                                Pickup Logistics
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <InputField
                        ...
                        <div className="flex flex-col gap-6 mt-4">
                            <div className="flex items-start gap-4 p-5 rounded-2xl bg-orange-50 border border-orange-100">
                                ...
                                <p className="text-sm font-medium text-slate-600 leading-relaxed">
                                    Your contact details are secure. We only share exact pickup locations with verified collection partners once a match is confirmed.
                                </p>
                            </div>
