...
const MOCK_ENQUIRY = {
    ...
    notes: 'Looking for a premium setup with live counters. Need special focus on the dessert section. No beef or pork on the menu.',
    attachments: [
        { id: 1, name: 'Venue_Floorplan.pdf', size: '2.4 MB', type: 'pdf', icon: FileText, color: 'text-blue-500', bg: 'bg-blue-50' },
        { id: 2, name: 'Reference_Decor.jpg', size: '1.1 MB', type: 'image', icon: ImageIcon, color: 'text-emerald-500', bg: 'bg-emerald-50' }
    ]
};
...
const MOCK_QUOTES = [
    {
        ...
        price: '₹5,50,000',
        perHead: '₹550 / head',
        message: 'Hi Priya, we would love to cater your Sangeet! We have attached our premium North Indian & Continental fusion menu. This includes 4 live counters and a specialized dessert bar as requested.',
        time: 'Today, 2:30 PM',
        isNew: true
    },
    {
        ...
        price: '₹4,80,000',
        perHead: '₹480 / head',
        message: 'Hello! I have reviewed your requirements. We can definitely manage the Sangeet within your budget. Please check out the attached proposal for the menu breakdown.',
...
const MOCK_ACTIVITY = [
    { id: 1, title: 'Quote Received', auth: 'Royal Feast Caterers', time: 'Today, 2:30 PM', type: 'success' },
    { id: 2, title: 'Caterer Viewed Enquiry', auth: 'Spice Route Kitchen', time: 'Yesterday, 8:00 PM', type: 'neutral' },
    { id: 3, title: 'Quote Received', auth: 'Spice Route Kitchen', time: 'Yesterday, 4:15 PM', type: 'success' },
    { id: 4, title: 'Enquiry Broadcasted', auth: 'System', time: 'Oct 15, 10:00 AM', type: 'neutral' },
    { id: 5, title: 'Enquiry Created', auth: 'Priya Sharma', time: 'Oct 15, 09:45 AM', type: 'neutral' }
];
        ...
        <div className="w-full py-6">
            <div className="flex items-center justify-between relative">
                ...
                <div className="absolute left-[10%] right-[10%] top-1/2 -translate-y-1/2 h-1 bg-slate-200 rounded-full z-0 hidden sm:block">
                    <div className="h-full bg-[#ef9d2a] rounded-full transition-all duration-500" style={{ width: '33%' }}></div>
                </div>

                {steps.map((step, idx) => (
                    <div key={step.id} className="relative z-10 flex flex-col items-center gap-2 bg-[#fcfaf8] sm:bg-transparent px-2">
                        ...
                        <span className={`text-[11px] sm:text-xs font-bold uppercase tracking-widest ${step.status === 'upcoming' ? 'text-slate-400' : 'text-stone-900'
                            }`}>
                            {step.label}
                        </span>
                    </div>
    ...
    <div className="bg-white rounded-[1.5rem] p-6 sm:p-8 shadow-sm border border-transparent flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="flex flex-col gap-1">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Event Type</span>
                <span className="text-base font-bold text-stone-900">{data.type}</span>
            </div>
            <div className="flex flex-col gap-1">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Guest Count</span>
                <span className="text-base font-bold text-stone-900">{data.guestCount}</span>
            </div>
            <div className="flex flex-col gap-1">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Budget Config</span>
            ...
            <div className="flex flex-col gap-1 sm:col-span-2">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Cuisine Requirements</span>
        ...
        <div className="flex flex-col gap-3 pt-4 border-t border-slate-100">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Additional Notes</span>
        ...
        {data.attachments && data.attachments.length > 0 && (
            <div className="flex flex-col gap-3 pt-4 border-t border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Attachments ({data.attachments.length})</span>
                <div className="flex flex-wrap gap-3">
                    {data.attachments.map(file => {
                        const Icon = file.icon;
                        return (
                            <div key={file.id} className="flex items-center gap-3 p-3 pr-4 rounded-xl border border-slate-200 hover:border-[#ef9d2a]/30 hover:bg-orange-50/20 transition-all cursor-pointer group">
                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${file.bg} ${file.color}`}>
                                    <Icon className="w-5 h-5" />
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-sm font-bold text-stone-900 group-hover:text-[#ef9d2a] transition-colors">{file.name}</span>
...
const QuoteCard = ({ quote }) => (
    <div className={`bg-white rounded-[1.5rem] p-6 sm:p-8 shadow-sm border transition-shadow hover:shadow-md ${quote.isNew ? 'border-[#ef9d2a]/30 ring-4 ring-[#ef9d2a]/5' : 'border-transparent'}`}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
            ...
            <div className="flex flex-col sm:items-end">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Estimated Quote</span>
        ...
        <div className="relative border-l-2 border-slate-100 ml-3 pl-6 space-y-8">
            {log.map((item, idx) => (
                <div key={item.id} className="relative">
                    ...
                    {/* Node Data */}
                    <div className="flex flex-col gap-1">
                        <span className={`text-sm font-bold ${item.type === 'success' ? 'text-stone-900' : 'text-stone-700'}`}>
                            {item.title}
                        </span>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2 text-sm font-bold text-slate-400">
                            <Link to="/dashboard/enquiries" className="hover:text-stone-900 transition-colors flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-4">
                            <h1 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">{MOCK_ENQUIRY.type}</h1>
                    ...
                    <div className="xl:col-span-2 flex flex-col gap-6 lg:gap-8">
                        ...
                        <div className="flex flex-col gap-6 mt-4">
                            <div className="flex items-center justify-between px-2">
                                <h3 className="text-xl font-black text-stone-900 tracking-tight">Caterer Responses</h3>
                                <span className="text-sm font-bold text-[#ef9d2a] px-3 py-1 bg-orange-50 rounded-full">{MOCK_QUOTES.length} Quotes Received</span>
                            </div>
