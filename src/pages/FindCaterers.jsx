...
const MOCK_CATERERS = [
    ...
    },
    {
        ...
        minOrder: '$200',
        pricePerPlate: '$15 - $30',
        image: 'https://images.unsplash.com/photo-1498837167922-41c53bbfcdcd?auto=format&fit=crop&q=80&w=800',
        isVerified: true,
        tags: ['Eco-Friendly']
    },
    {
    ...
    },
    {
        ...
        minOrder: '$150',
        pricePerPlate: '$12 - $25',
        image: 'https://images.unsplash.com/photo-1530103862676-de8892b07d5b?auto=format&fit=crop&q=80&w=800',
        isVerified: false,
        tags: ['Home Chef']
    },
    {
...
const CUISINES = ['North Indian', 'South Indian', 'Chinese', 'Italian', 'Mexican', 'Vegan', 'Desserts', 'Mediterranean', 'BBQ'];
const EVENT_TYPES = ['Weddings', 'Corporate', 'Birthdays', 'Private Parties', 'Casual', 'Galas'];

// --- SUB-COMPONENTS ---

const FilterSidebar = ({ onReset }) => {
    const [selectedCuisines, setSelectedCuisines] = useState(['North Indian', 'Chinese']);
    const [selectedEvents, setSelectedEvents] = useState(['Weddings']);
    ...
    return (
        <div className="bg-white rounded-[2rem] p-6 lg:p-8 shadow-sm border border-transparent sticky top-6">
            ...
            <div className="mb-8">
                <h3 className="text-sm font-bold text-stone-900 mb-4 uppercase tracking-wider">Cuisine Type</h3>
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-bold text-stone-900 uppercase tracking-wider">Max Price / Plate</h3>
            ...
            {/* Event Types */}
            <div>
                <h3 className="text-sm font-bold text-stone-900 mb-4 uppercase tracking-wider">Event Category</h3>
                <div className="flex flex-col gap-3">
                    {EVENT_TYPES.map(event => {
                        const isSelected = selectedEvents.includes(event);
                        return (
                            <label key={event} className="flex items-center gap-3 cursor-pointer group">
                                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${isSelected ? 'bg-[#ef9d2a] border-[#ef9d2a]' : 'border-stone-300 group-hover:border-[#ef9d2a]'
                                    }`}>
                                    {isSelected && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                                </div>
                                <span className={`text-sm font-medium ${isSelected ? 'text-stone-900' : 'text-stone-600'}`}>{event}</span>
                            </label>
                        );
                    })}
                </div>
                ...
                {caterer.tags.length > 0 && (
                    <div className="absolute bottom-4 left-4 flex gap-2">
                        {caterer.tags.map(tag => (
                            <span key={tag} className="bg-[#ef9d2a] text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md shadow-sm">
                                {tag}
                            </span>
                        ))}
                    </div>
            ...
            <div className="p-6 md:p-8 flex flex-col flex-1 gap-5">
                ...
                <div className="flex flex-col gap-1.5 border-b border-stone-100 pb-5">
                    <div className="flex items-start justify-between gap-2">
                        <h3 className="text-xl font-black text-stone-900 leading-tight group-hover:text-[#ef9d2a] transition-colors line-clamp-1">{caterer.name}</h3>
                        {caterer.isVerified && (
                            <div className="w-6 h-6 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center shrink-0" title="Verified Caterer">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-stone-50 text-stone-600 text-xs font-bold border border-stone-100">
                        ...
                        <span className="line-clamp-1 max-w-[100px]">{caterer.eventTypes[0]} {caterer.eventTypes.length > 1 && '+'}</span>
                    </div>
                </div>
                ...
                <div className="mt-auto flex items-end justify-between pt-2">
                    <div className="flex flex-col">
                        <span className="text-[10px] uppercase tracking-widest text-stone-400 font-bold mb-0.5">Price per plate</span>
                        <span className="text-lg font-black text-stone-900">{caterer.pricePerPlate}</span>
                    </div>
                    <div className="flex flex-col items-end">
                        <span className="text-[10px] uppercase tracking-widest text-stone-400 font-bold mb-0.5">Min Order</span>
        ...
        <div className="min-h-screen bg-[#fcfaf8] font-['Plus_Jakarta_Sans',sans-serif] selection:bg-[#ef9d2a]/30">
            ...
            <header className="bg-white border-b border-stone-100 h-20 flex items-center px-6 lg:px-12 justify-between sticky top-0 z-50">
                ...
                <nav className="hidden md:flex items-center gap-8">
                    <Link to="/" className="text-sm font-bold text-stone-500 hover:text-stone-900 transition-colors">Home</Link>
                    <Link to="/caterers" className="text-sm font-black text-[#ef9d2a]">Find Caterers</Link>
                    <Link to="/donate-food" className="text-sm font-bold text-stone-500 hover:text-stone-900 transition-colors">Donate Food</Link>
                </nav>
                <div className="flex items-center gap-4">
                    <Link to="/login" className="text-sm font-bold text-stone-600 hover:text-stone-900 hidden sm:block">Log In</Link>
                    <Link to="/signup" className="px-6 py-2.5 rounded-full bg-stone-900 text-white text-sm font-bold hover:bg-stone-800 transition-colors shadow-md">Sign Up</Link>
                </div>
            </header>
                ...
                <div className="flex-1 w-full flex flex-col min-w-0 pb-20">
                    ...
                    <div className="bg-white rounded-[2rem] p-3 shadow-sm border border-stone-100 flex items-center mb-8 relative z-20">
                        ...
                        <div className="hidden md:flex flex-1 items-center px-8 gap-3">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase text-stone-400 tracking-wider">Guest Count</span>
                    ...
                    <div className="flex items-center justify-between mb-6 px-2">
                        <h2 className="text-2xl font-black text-stone-900 tracking-tight">Top Rated Caterers <span className="text-stone-400 font-medium text-lg ml-2">{MOCK_CATERERS.length} found</span></h2>
