        ...
        <div className="flex flex-col xl:flex-row gap-4 xl:items-center justify-between bg-white p-2 rounded-2xl shadow-sm border border-stone-100">
            ...
            <div className="flex flex-wrap items-center gap-2 px-2 pb-2 xl:pb-0">
                <button className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-white border border-stone-200 text-stone-700 hover:border-[#ef9d2a] hover:text-[#ef9d2a] transition-colors text-sm font-medium group">
                    <span>Status: <span className="text-stone-900 font-bold group-hover:text-[#ef9d2a]">All</span></span>
                ...
                {/* Clear Filters */}
                <button className="p-2.5 ml-1 rounded-full text-stone-400 hover:text-red-500 hover:bg-red-50 transition-colors" title="Clear Filters">
        ...
        <div className="bg-white rounded-[2rem] border border-stone-100 shadow-sm overflow-hidden animate-fade-in delay-100">
                ...
                <table className="w-full text-left border-collapse whitespace-nowrap min-w-[900px]">
                    <thead>
                        <tr className="border-b border-stone-100 bg-stone-50/50">
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider w-32">Ref ID</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider">Food Type</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider">Quantity</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider">Location</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider text-center">Urgency</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider text-center">Status</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider hidden lg:table-cell">Note</th>
                            <th className="px-6 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                        {data.map((req, idx) => (
                            <tr key={idx} className="group hover:bg-[#ef9d2a]/[0.05] transition-colors cursor-default">
                                <td className="px-6 py-5">
                                    <span className="text-sm font-bold text-stone-500 font-mono tracking-tight">{req.id}</span>
                                </td>
                                <td className="px-6 py-5">
                                ...
                                <td className="px-6 py-5 hidden lg:table-cell">
                                    <p className="text-xs font-medium text-stone-400 max-w-[160px] truncate">{req.note}</p>
                                </td>
                                <td className="px-6 py-5 text-right">
                                    {/* Action Button */}
                                    <Link to={`/caterer/requests/${req.id.replace('#REQ-', '')}`} className="inline-flex items-center gap-2 justify-center h-10 px-6 rounded-full border border-stone-200 text-stone-600 text-xs font-bold hover:bg-[#ef9d2a] hover:text-white hover:border-[#ef9d2a] transition-all shadow-sm">
            ...
            {data.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                    ...
                    <p className="text-stone-500 max-w-sm mx-auto mb-8 font-medium">It looks like there are no active food requests matching your current filters.</p>
                </div>
            )}
            ...
            {data.length > 0 && (
                <div className="flex items-center justify-between border-t border-stone-100 bg-stone-50/50 px-6 py-4">
                    <p className="text-sm text-stone-500 font-medium">
                        Showing <span className="font-bold text-stone-900">1</span> to <span className="font-bold text-stone-900">{data.length}</span> of <span className="font-bold text-stone-900">{data.length}</span> results
                    </p>
                </div>
            )}
        </div>
            ...
            <div className="flex-1 flex flex-col gap-8 w-full">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in-up">
                    <div className="flex flex-col gap-1">
                        <h2 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">Food Requests</h2>
                        <p className="text-stone-500 font-medium text-lg">Manage and track community food assistance workflows.</p>
                    </div>
