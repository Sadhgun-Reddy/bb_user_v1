...
const AddNewPlaceholder = () => {
  return (
    <Link to="/caterer/menus/new" className="group bg-transparent rounded-2xl p-6 border-2 border-dashed border-stone-200 flex flex-col items-center justify-center min-h-[360px] cursor-pointer hover:border-[#ef9d2a]/50 hover:bg-[#ef9d2a]/5 transition-all">
...
const MenuManagement = () => {
  ...
  const { items: menus, isLoading, error } = useSelector((state) => state.menu);
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf8] font-['Plus_Jakarta_Sans',sans-serif] text-stone-900 selection:bg-[#ef9d2a]/30 p-4 sm:p-8 w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 animate-fade-in-up">
        ...
        <Link to="/caterer/menus/new" className="bg-[#ef9d2a] hover:bg-[#d98a1e] text-white px-6 py-3 rounded-full font-bold shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2 transition-all transform hover:-translate-y-0.5 active:translate-y-0 shrink-0">
      ...
      {isLoading && menus.length === 0 ? (
        <div className="flex justify-center items-center h-64">
          <div className="w-10 h-10 border-4 border-stone-200 border-t-[#ef9d2a] rounded-full animate-spin"></div>
        </div>
      ) : (
