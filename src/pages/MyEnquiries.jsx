...
const STATS = [
    ...
    { label: 'RESPONSES', value: '5', icon: MessageSquare, color: 'text-orange-500', bg: 'bg-orange-50' },
...
const MOCK_ENQUIRIES = [
    ...
    },
    {
        ...
        catererAvatar: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=100&h=100&fit=crop',
        catererInitials: null,
...
const getStatusStyles = (status) => {
    switch (status) {
        ...
        case 'Pending': return 'text-[#ef9d2a] bg-orange-50 border-orange-200';
        case 'Viewed': return 'text-blue-600 bg-blue-50 border-blue-200';
        case 'Closed': return 'text-stone-500 bg-stone-50 border-stone-200';
        ...
        <div className="w-full animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
                <div className="flex flex-col gap-2 max-w-2xl">
                    <h1 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">My Enquiries</h1>
                    <p className="text-stone-500 font-medium text-lg leading-snug">
                        Track the status of your catering requests, manage proposals, and connect with the best bawarchis for your events.
                    </p>
                </div>
                <Link
                    to="/caterers"
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10">
                {STATS.map((stat, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-white rounded-[2rem] p-6 shadow-sm border border-stone-100 flex items-center justify-between hover:shadow-md transition-shadow">
                            <div className="flex flex-col gap-1">
                                <span className="text-xs font-black uppercase tracking-widest text-stone-400">{stat.label}</span>
                ...
                <table className="w-full min-w-[1000px] text-left border-collapse">
                    <thead>
                        <tr className="border-b border-stone-100 bg-stone-50/30">
                            <th className="py-5 px-8 text-xs font-black text-stone-400 uppercase tracking-widest">Reference</th>
                            <th className="py-5 px-6 text-xs font-black text-stone-400 uppercase tracking-widest">Event Type</th>
                            <th className="py-5 px-6 text-xs font-black text-stone-400 uppercase tracking-widest">Caterer</th>
                            <th className="py-5 px-6 text-xs font-black text-stone-400 uppercase tracking-widest">Event Date</th>
                            <th className="py-5 px-6 text-xs font-black text-stone-400 uppercase tracking-widest text-center">Guests</th>
                            <th className="py-5 px-6 text-xs font-black text-stone-400 uppercase tracking-widest text-center">Status</th>
                            <th className="py-5 px-8 text-xs font-black text-stone-400 uppercase tracking-widest text-center">Action</th>
                        </tr>
                    </thead>
                                ...
                                <tr key={idx} className="hover:bg-stone-50/50 transition-colors group">
                                    ...
                                    <td className="py-5 px-6 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-full ${enquiry.iconBg} ${enquiry.iconColor} flex items-center justify-center shadow-sm`}>
                                                <EventIcon className="w-5 h-5" />
                                            </div>
                                            <span className="text-[15px] font-black text-stone-900 group-hover:text-[#ef9d2a] transition-colors">{enquiry.eventType}</span>
                                        </div>
                                    </td>
                                        ...
                                        <div className="flex items-center gap-3">
                                            {!enquiry.isMultiple ? (
                                                enquiry.catererAvatar ? (
                                                    <img src={enquiry.catererAvatar} alt={enquiry.caterer} className="w-10 h-10 rounded-full object-cover border border-stone-200" />
                                                ) : (
                                            ...
                                            <span className={`text-[15px] font-bold ${enquiry.isMultiple ? 'text-stone-400 italic' : 'text-stone-700'}`}>
                                                {enquiry.caterer}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    {/* Status */}
                                    <td className="py-5 px-6 whitespace-nowrap text-center">
                                        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border ${getStatusStyles(enquiry.status)}`}>
                                            <div className={`w-2 h-2 rounded-full ${getStatusDotColor(enquiry.status)}`}></div>
            ...
            <div className="flex items-center justify-between px-4 mt-6">
                <span className="text-sm font-medium text-stone-500">Showing <strong className="text-stone-800">1–4</strong> of <strong className="text-stone-800">12</strong> enquiries</span>
