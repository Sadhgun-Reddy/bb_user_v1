    ...
    <div className="flex items-center justify-between mb-12 relative px-2">
      ...
      {steps.map((step) => (
        <div key={step.num} className="relative z-10 flex flex-col items-center gap-2">
          ...
          <span
            className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider absolute -bottom-7 w-max transition-colors ${currentStep >= step.num ? 'text-[#ef9d2a]' : 'text-stone-400'
              }`}
          >
            {step.label}
          </span>
...
// 2. Step 1: Details
const Step1Details = ({ formData, setFormData, onNext }: any) => {
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center mb-10">
        ...
        <p className="text-stone-500 mt-2 font-medium text-lg">
          We connect community kitchens with those in need. Tell us how we can help.
        </p>
      </div>
      ...
      <div className="space-y-3 pt-2">
        ...
        {formData.urgency === 'Critical' && (
          <div className="flex items-start gap-2 text-red-600 bg-red-50 p-4 rounded-[1.5rem] text-sm font-medium mt-3 animate-fade-in border border-red-100">
            ...
            <p>
              We will try to prioritize this request and notify nearby partners immediately. Expect
              a faster callback.
            </p>
          </div>
        )}
      ...
      <div className="space-y-2 pt-2">
        <label className="ml-1 text-sm font-bold text-stone-700">
...
const Step2Verification = ({ formData, setFormData, onNext, onBack }: any) => {
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">
        ...
        <p className="text-stone-500 mt-2 font-medium text-lg">
          Please provide your contact info so we can coordinate the pickup/delivery.
        </p>
      </div>
      ...
      <div className="bg-[#ef9d2a]/5 border border-[#ef9d2a]/20 rounded-3xl p-6 mt-4">
        ...
        <p className="text-sm font-medium text-[#b57315]/80">
          Our partner kitchens will use this number to verify the request details and coordinate
          logistics directly with you.
        </p>
      </div>
...
// 4. Step 3: Delivery
const Step3Delivery = ({ formData, setFormData, onSubmit, onBack }: any) => {
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-black text-stone-900 tracking-tight">
        ...
        <p className="text-stone-500 mt-2 font-medium text-lg">
          When exactly do you need the assistance?
        </p>
      </div>
      ...
      <div className="bg-stone-50 rounded-[1.5rem] p-6 border border-stone-200 mt-8">
        ...
        <ul className="space-y-3 text-sm text-stone-600 font-medium">
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong className="text-stone-900">For:</strong> {formData.guestCount || '0'} People (
          ...
          <li className="flex items-start gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#ef9d2a] mt-1.5 shrink-0"></span>
            <span>
              <strong className="text-stone-900">To:</strong> {formData.location || 'Not provided'}
            </span>
          </li>
          <li className="flex items-start gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#ef9d2a] mt-1.5 shrink-0"></span>
            <span>
              <strong className="text-stone-900">By:</strong> {formData.name || 'Not provided'} (
              {formData.phone || 'N/A'})
            </span>
          </li>
        </ul>
