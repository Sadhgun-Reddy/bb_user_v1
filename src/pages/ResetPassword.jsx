...
const ResetPassword = () => {
    ...
    const getPasswordStrength = (pass) => {
        ...
        if (pass.length < 6) return { score: 1, label: 'Weak', color: 'bg-red-500', textClass: 'text-red-500' };
        if (pass.length < 8 || !/\d/.test(pass)) return { score: 2, label: 'Fair', color: 'bg-yellow-500', textClass: 'text-yellow-600' };
            ...
            <main className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden">
                ...
                <div className="w-full max-w-md bg-white rounded-[2rem] sm:rounded-3xl shadow-2xl shadow-stone-200/50 border border-stone-100 p-8 sm:p-10 relative z-10 animate-fade-in-up">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-[#1b160d] mb-3 tracking-tight">Set New Password</h1>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="ml-1 text-xs font-bold uppercase tracking-wider text-stone-500">New Password</label>
                            ...
                            <div className="flex items-center gap-3 mt-3 px-2">
                                <div className="flex-1 h-[6px] bg-stone-100 rounded-full overflow-hidden flex">
                                    <div className={`h-full transition-all duration-300 w-1/3 border-r border-white/50 ${strength.score >= 1 ? strength.color : 'bg-transparent'}`}></div>
                                    <div className={`h-full transition-all duration-300 w-1/3 border-r border-white/50 ${strength.score >= 2 ? strength.color : 'bg-transparent'}`}></div>
                                    <div className={`h-full transition-all duration-300 w-1/3 ${strength.score >= 3 ? strength.color : 'bg-transparent'}`}></div>
                                </div>
                                <span className={`text-xs font-bold w-12 text-right ${strength.textClass}`}>
                                    {strength.label}
                                </span>
                            </div>
                            <p className="text-xs text-stone-400 font-medium px-2 pt-1">Must be at least 8 characters.</p>
                        </div>

                        {/* Confirm Password Field */}
                        <div className="space-y-1.5 mt-2">
                            <label className="ml-1 text-xs font-bold uppercase tracking-wider text-stone-500">Confirm Password</label>
                            <div className="relative group">
                                <input
                                    ...
                                    required
                                    className={`w-full h-12 px-6 pr-12 bg-stone-50 border rounded-full text-[#1b160d] placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-[#ef9d2a]/50 focus:border-[#ef9d2a] transition-all font-medium ${confirmPassword.length > 0 && password !== confirmPassword ? 'border-red-300 focus:border-red-400 focus:ring-red-500/50' : 'border-stone-200'
                                        }`}
                                    placeholder="••••••••"
                                />
                                <button
                        ...
                        <div className="text-center mt-2 pt-2">
                            <button onClick={() => navigate('/login')} type="button" className="inline-flex items-center gap-2 text-sm font-bold text-stone-500 hover:text-[#ef9d2a] transition-colors group">
