...
const TicketHeader = () => (
    <div className="bg-white rounded-[2rem] p-8 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-sm border border-transparent">
        <div className="flex flex-col gap-1.5">
            <h1 className="text-2xl lg:text-3xl font-black text-stone-900 tracking-tight">Need help with an order?</h1>
            <p className="text-slate-500 font-medium">Create a new ticket and our support team will assist you within 24 hours.</p>
        </div>
...
const TicketTable = () => (
    <div className="w-full overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
                <tr>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400 pl-4 w-32">Ticket ID</th>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400">Subject</th>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400">Category</th>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400">Status</th>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400">Last Updated</th>
                    <th className="pb-4 border-b border-slate-100 uppercase text-[10px] tracking-wider font-bold text-slate-400 text-right pr-4">Action</th>
                </tr>
            </thead>
            <tbody>
                {MOCK_TICKETS.map((ticket, index) => (
                    <tr key={index} className="group hover:bg-slate-50/50 transition-colors cursor-pointer border-b border-slate-50 last:border-0">
                        ...
                        <td className="py-5 pr-8 align-top">
                            <div className="flex flex-col gap-1">
                                <span className="font-bold text-stone-900 group-hover:text-[#ef9d2a] transition-colors line-clamp-1">{ticket.subject}</span>
                                <span className="text-xs font-medium text-slate-400 line-clamp-1">{ticket.snippet}</span>
                            </div>
                        </td>

                        <td className="py-5 align-top">
                            <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-black uppercase tracking-wider ${ticket.categoryColor}`}>
                                {ticket.category}
                            </span>
                        </td>
        ...
        <div className="flex items-center justify-between mt-6 pt-6 border-t border-slate-100">
            <span className="text-sm font-semibold text-slate-500 ml-4">Showing 1 to 5 of 12 tickets</span>
            <div className="flex items-center gap-2 mr-4">
                <button className="w-8 h-8 rounded-full flex items-center justify-center border border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-stone-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled>
